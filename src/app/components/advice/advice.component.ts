...
export class AdviceComponent implements OnInit, OnDestroy {
  ...
  displayedColumns = ['category', 'title', '_id'];
  ...
  getData() {
    ...
    this.querySubscription = this._backendService.getAdvice('advice')
      .subscribe(members => {
...
// filter as you search
  getFilterData(filters) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.getFilterProducts('advice', filters)
      .subscribe(members => {
        this.members = members;
        this.dataSource = new MatTableDataSource(members);
        this.dataSource.paginator = this.paginator;
  ...
  setData(formData) {
    this.dataLoading = true;
    //was setNewDoc
    this._backendService.setItem('advice', formData).then((res) => {
      this.dataLoading = false;
  ...
  updateData(formData) {
    ...
    if (confirm("Are you sure want to update this record ?")) {
      this.dataLoading = true;
      this._backendService.updateAdvice('advice', formData).then((res) => {
        this.error = false;
        this.errorMessage = "";
        this.dataLoading = false;
        this.savedChanges = true;
  ...
  getDoc(docId) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.getSingleAdvice(docId)
      .subscribe(res => {
        if (res) {
          this.myDocData = res;
  ...
  deleteDoc(record_id) {
    if (confirm("Are you sure you want to delete this record ?")) {
      this.dataLoading = true;
      this._backendService.deleteAdvice(record_id);
