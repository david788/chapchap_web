...
export class BackendService {
  ...
  //collections to be fetched from firebase and displayed on tables
  private _firebaseCollURL = "disasters";
  private _firebaseIncidencesCollURL = "incidences";
  private _firebaseContactsCollURL = "contacts";
  private _firebaseAdviceCollURL = "advice";
  private _firebaseUsersCollURL = "users";
  ...
  //CREATE
  setItem(coll: string, formData: any, docId?: string) {
  ...
  setNewDoc(coll: string, data: any, docId?: any) {
    ...
    const item = { id, name };
    const timestamp = this.timestamp
    const time = this.timestamp.toTimeString();
  ...
  //the function to upload a pic for a single incidence
  setProductPic(filePath, coll, docId?) {
    var docRef = this.afs.collection('incidences').doc(docId);
    return docRef.set({
      path: filePath
    }, { merge: true });
  }
  ...
  //single reported disaster
  getDisaster(docId: string) {
    return this.getDisasterDoc(docId);
  }
  getDisasterDoc(docId: string) {
    return this.afs.collection('disasters').doc(docId).valueChanges();
  }
  //single incidence
  getIncidence(docId: string) {
    return this.getIncidenceDoc(docId);
  }
  getIncidenceDoc(docId: string) {
    return this.afs.collection('incidences').doc(docId).valueChanges();
  }

  //single contact
  getContact(docId: string) {
    return this.getContactDoc(docId);
  }
  getContactDoc(docId: string) {
    return this.afs.collection('contacts').doc(docId).valueChanges();
  }

  //single advice
  getSingleAdvice(docId: string) {
    return this.getSingleAdviceDoc(docId);
  }
  getSingleAdviceDoc(docId: string) {
    return this.afs.collection('advice').doc(docId).valueChanges();
  }



  //functions to get collection documents and display on a table
  //for reported table
  ...
  //for incidences table
  getIncidences(filters?: any) {
    this.nameFilter$ = new BehaviorSubject(null);
    this.rollNoFilter$ = new BehaviorSubject(null);
    ...
    return this.items$ = Observable.combineLatest(
      ...
      ([STD_NM, ROLLNO]) =>
        this.afs.collection<Item>(this._firebaseIncidencesCollURL, ref => {
          let query = ref.where('delete_flag', '==', 'N');
          if (STD_NM) { query = ref.where('STD_NM', '>=', STD_NM); query = query.orderBy('STD_NM', 'desc'); };
          if (ROLLNO) { query = ref.where('ROLLNO', '>=', ROLLNO); query = query.orderBy('ROLLNO', 'desc'); };
          return query;
  ...
  getContacts(filters?: any) {
    ...
    return this.items$ = Observable.combineLatest(
      ...
      ([STD_NM, ROLLNO]) =>
        this.afs.collection<Item>(this._firebaseContactsCollURL, ref => {
          let query = ref.where('delete_flag', '==', 'N');
          if (STD_NM) { query = ref.where('STD_NM', '>=', STD_NM); query = query.orderBy('STD_NM', 'desc'); };
          if (ROLLNO) { query = ref.where('ROLLNO', '>=', ROLLNO); query = query.orderBy('ROLLNO', 'desc'); };
          return query;
  ...
  getAdvice(filters?: any) {
    ...
    return this.items$ = Observable.combineLatest(
      ...
      ([STD_NM, ROLLNO]) =>
        this.afs.collection<Item>(this._firebaseAdviceCollURL, ref => {
          let query = ref.where('delete_flag', '==', 'N');
          if (STD_NM) { query = ref.where('STD_NM', '>=', STD_NM); query = query.orderBy('STD_NM', 'desc'); };
          if (ROLLNO) { query = ref.where('ROLLNO', '>=', ROLLNO); query = query.orderBy('ROLLNO', 'desc'); };
          return query;
  ...
  getUsers(filters?: any) {
    ...
    return this.items$ = Observable.combineLatest(
      ...
      ([STD_NM, ROLLNO]) =>
        this.afs.collection<Item>(this._firebaseUsersCollURL, ref => {
          let query = ref.where('delete_flag', '==', 'N');
          if (STD_NM) { query = ref.where('STD_NM', '>=', STD_NM); query = query.orderBy('STD_NM', 'desc'); };
          if (ROLLNO) { query = ref.where('ROLLNO', '>=', ROLLNO); query = query.orderBy('ROLLNO', 'desc'); };
          return query;
  ...
  updateDisaster(coll, formData) {
    return this.updateDoc("disasters", formData._id, formData);
  }
  updateIncidence(coll, formData) {
    return this.updateDoc("incidences", formData._id, formData);
  }
  updateContact(coll, formData) {
    return this.updateDoc("contacts", formData._id, formData);
  }
  updateAdvice(coll, formData) {
    return this.updateDoc("advice", formData._id, formData);
  }
  ...
  deleteOneProduct(record_id) {
    ...
    data["delete_flag"] = "Y"
    this.fireservices.doc('disasters/' + record_id).update(data);
  }
  deleteOneIncidence(record_id) {
    let data = {};
    data["updatedAt"] = this.timestamp;
    data["delete_flag"] = "Y"
    this.fireservices.doc('incidences/' + record_id).update(data);
  }

  deleteOneContact(record_id) {
    let data = {};
    data["updatedAt"] = this.timestamp;
    data["delete_flag"] = "Y"
    this.fireservices.doc('contacts/' + record_id).update(data);
  }
  deleteAdvice(record_id) {
    let data = {};
    data["updatedAt"] = this.timestamp;
    data["delete_flag"] = "Y"
    this.fireservices.doc('advice/' + record_id).update(data);
  }

  //the function to delete the picture of a single document
  deleteProductPic(record_id) {
    this.fireservices.doc('incidences/' + record_id).set({
      path: null
    }, { merge: true });
  }
